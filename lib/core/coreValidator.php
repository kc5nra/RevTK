 ...
 * @copyright  Some code based on Symfony php framework, by Fabien Potencier (www.symfony-project.org)
 */
	...
	public function validate($data)
	{
		...
		foreach($this->fields as $field => $rules)
		{
			...
			foreach ($rules as $ruleName => $ruleParams)
			{
				...
				{
					throw new coreException("coreValidator rule '{$ruleName}' is not a Validator.");
				}
				...
				if (method_exists($this, $ruleName))
				{
					...
					if ($result!==true)
					{
						if (is_null($result))
						{
							throw new coreException('Error message not set in validation file for '.$field);
						}
				...
				{
					throw new coreException("Validator '{$ruleName}' not found in class ".get_class($this));
				}
			}
		}
		...
		if (!$valid)
		{
			coreContext::getInstance()->getRequest()->setErrors($this->errors);
		}
