...
class uiSelectTable
{
  ...
  private function configureSettings($settings)
  {
    foreach ($settings as $name => $value)
    {
      ...
      {
        throw new coreException("Unknown parameter '{$name}' in uiSelectTable settings.");
      }
  ...
  private function verifyRowIds(array $rowids)
  {
    foreach($rowids as $id) {
      if ($id!==self::POSTDATA_NEW_ROWID && !$this->isValidRowId($id)) {
        throw new coreException(__CLASS__.'::'.__METHOD__.'Unexpected row id!');
      }
    }
  }
    ...
    if ($num_rows>0)
    {
      foreach ($this->columns as $colDef)
      {
        if ($colDef->editable)
        {
          ...
          if ($colData===null) {
            throw new coreException(__CLASS__.":: column data for '{$colDef->colData}' missing");
          }
          $postCols[$colDef->colData] = $colData;
          if (count($colData)!==$num_rows) {
            throw new coreException(__CLASS__.":: column data incomplete");
          }
        }
      }
...
EOD;
  ...
  private function getColHead($colDef)
  {
    ...
    if ($this->sortColumn == $colDef->colSort) {
      $classAttr = 'class="active sort'.strtolower($this->sortOrders[$this->sortOrder]).'" ';
      $next_order = $this->sortOrder ? 0 : 1;
    }
    elseif (isset($colDef->colSort))
    {
      $classAttr = 'class="sort" ';
      $next_order = 0;
    }
